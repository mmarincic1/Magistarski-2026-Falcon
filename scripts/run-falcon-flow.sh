...
echo "=== Falcon agg - different roots - fallback strategy ==="
echo

echo "[0/9] Cleaning previous run files..."
rm -f \
...
echo "[1/9] Generating issuer keys..."
...
npm run create-credential -- \
  ...
  --key IssuerB_falcon_privateKey.json
...
npm run verify-single -- \
  ...
  --key IssuerB_falcon_publicKey.json \
  --required VerifierCIssuerBRequiredDisclosures.json

echo "[7/9] Creating aggregated presentation..."
...
echo "[8/9] Verifying aggregated presentation..."
...
echo
echo "✅ Falcon fallback flow completed successfully."